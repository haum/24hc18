...
def nest_strat(n):
    ...
    output.append(f'SET_MEMORY {memstring}')
    return output
...
def ant_strat(a):
    ...
    if a.typ == 0:  # explorers
        ...
        self_pheromones = [_['id'] for _ in a.pheromones if _['type'] == ant_id]
        try:
            target_pheromone = sorted(self_pheromones, key=lambda _: _.get('dist'))[-1]
        except:
            target_pheromone = False

        try:
            current_pheromone = sorted(self_pheromones, key=lambda _: _.get('dist'))[0]
        except:
        ...
        if a.memory[1] > 0:
            if moved_last:
                if found_food_last_time:
                    ...
                    output.append(f'CHANGE_PHEROMOME {current_pheromone} 10')
                    excl = True
                    a.memory[1] = 1
        ...
        pheromones = [_['id'] for _ in a.pheromones if _['typ'] == 10]
        try:
